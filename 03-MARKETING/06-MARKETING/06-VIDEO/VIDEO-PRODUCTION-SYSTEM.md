...
|---------|------|-------------|
| **Road laptop (Jordyn)** | Content + cloud video | Write, post, Canva carousels, HubSpot, LinkedIn, email, HeyGen, ElevenLabs |
| **HP Gaming PC (home)** | Local render only | Python/FFmpeg render pipeline, batch downloads, storage-heavy sessions |

**Rule:** HeyGen, ElevenLabs, and Canva are fully cloud-based — they run in any browser on any machine. You do NOT need the gaming PC for these. The only reason to use the gaming PC is if you're running the local Python render script (`lmt-video-overlay.py`) or doing a large batch download session where storage matters.
...
### Step 2: HEYGEN (browser — any machine)
...
### Step 3: ELEVENLABS (browser — any machine)
...
### Step 4: CANVA B-ROLL (browser — any machine)
  ...
  "gap_slides": [
    {
      ...
      "start": 33, "end": 45,
      "bullets": ["9 In-Person Locations", "MBE Certified", "WIOA Eligible"]
    },
    {
...
- Script: Based on Article 3 (9 In-Person Locations) or Article 5 (Deploy This Quarter)
...
| HeyGen talking head | Yes (cloud/browser) | Yes |
| ElevenLabs voiceover | Yes (cloud/browser) | Yes |
| Build Canva video (with b-roll) | Yes (cloud/browser) | Yes |
| Claude Code video render (local Python) | No — needs FFmpeg installed | Yes |
| Batch produce + store multiple videos | No — storage limits | Yes |
